...
//KMP + DFS + Memo
//use 4-D vector instead of unordered_map to speed up
...
//Runtime: 108 ms, faster than 43.06% of C++ online submissions for Find All Good Strings.
//Memory Usage: 18.5 MB, less than 100.00% of C++ online submissions for Find All Good Strings.
class Solution {
public:
    int n;
    int m;
    ...
    int count(int idx, bool pre1, bool pre2, int preE){
        ...
        if(memo[idx][preE][pre1][pre2] != -1){
            return memo[idx][preE][pre1][pre2];
        }
    ...
    int findGoodStrings(int n, string s1, string s2, string evil) {
        ...
        this->m = evil.size();
        this->mod = 1e9+7;
        this->memo = vector(n+1, vector(evil.size()+1, vector(2, vector(2, -1))));
        computeLPSArray();
